...
char* wtv_probe(const char* url, int* cancelled) {
    ...
    int offset = snprintf(output, capacity, "{\"formatName\":\"%s\",\"duration\":%.3f,\"bitrate\":%lld,\"streams\":[", format->iformat ? format->iformat->name : "", format->duration == AV_NOPTS_VALUE ? 0.0 : (double)format->duration / AV_TIME_BASE, (long long)format->bit_rate);
    for (unsigned int i = 0; i < format->nb_streams; i++) {
        AVStream* stream = format->streams[i];
        AVCodecParameters* codec = stream->codecpar;
        if (i > 0) output[offset++] = ',';
        offset += snprintf(output + offset, capacity - offset, "{\"index\":%d,\"codecName\":\"%s\",\"codecType\":\"%s\",\"width\":%d,\"height\":%d,\"frameRate\":%.3f,\"pixelFormat\":\"%s\"}", stream->index, avcodec_get_name(codec->codec_id), av_get_media_type_string(codec->codec_type), codec->width, codec->height, ratio_to_double(stream->avg_frame_rate.num ? stream->avg_frame_rate : stream->r_frame_rate), av_get_pix_fmt_name(codec->format));
    }
...
static int write_packet(void* opaque, uint8_t* buffer, int buffer_size) {
    int fd = *(int*)opaque;
    return (int)write(fd, buffer, buffer_size);
}
...
int wtv_remux(const char* url, int fd, int* cancelled) {
    ...
    AVIOContext* avio = avio_alloc_context(buffer, 32 * 1024, 1, &output_fd, NULL, write_packet, NULL);
    if (!avio) goto fail;
    output->pb = avio;
    output->flags |= AVFMT_FLAG_CUSTOM_IO;
    if (avformat_write_header(output, NULL) < 0) goto fail_io;

    AVPacket packet;
    av_init_packet(&packet);
    while (av_read_frame(input, &packet) >= 0) {
        packet.pts = av_rescale_q_rnd(packet.pts, input->streams[packet.stream_index]->time_base, output->streams[packet.stream_index]->time_base, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
        packet.dts = av_rescale_q_rnd(packet.dts, input->streams[packet.stream_index]->time_base, output->streams[packet.stream_index]->time_base, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
        packet.duration = av_rescale_q(packet.duration, input->streams[packet.stream_index]->time_base, output->streams[packet.stream_index]->time_base);
